...
int main() {
    ...
    #endif
    #ifndef ONLINE_JUDGE
    FILE* in = fopen("51input.bin", "rb");
    FILE* out = fopen("51output.bin", "wb");

    /* seed */
    uint8_t seed[32] = {
        0x5F, 0x7F, 0x00, 0x3F, 0x9F, 0x1C, 0x95, 0xF2, 0x90, 0x33, 0x53, 0x90, 0x3F, 0xAC, 
        0xC8, 0x18, 0xD2, 0xF8, 0x52, 0x97, 0xDB, 0x4B, 0xB6, 0xD1, 0xB8, 0xAA, 0x27, 0x79, 0x42, 0xC0, 0x78, 0x59
    };
    #else
    #define in stdin
    #define out stdout
    ...
    if (f) {
        size_t result = std::fread(seed, 1, 32, f);
        std::fclose(f);
        if (result != 32) {
            std::cerr << "Warning: Failed to read enough random bytes." << std::endl;
        }
    } 
    else {
    ...
    fread(ignore1, sizeof(uint8_t), 16, in);
    fread(n, sizeof(uint64_t), 32, in);
    fread(e, sizeof(uint64_t), 32, in);
    fread(ignore2, sizeof(uint8_t), 256, in);

    uint8_t m_len;
    fread(&m_len, sizeof(uint8_t), 1, in);
    uint8_t m[m_len];
    fread(m, sizeof(uint8_t), m_len, in);

    uint8_t DB[223] = {0};
    memcpy(DB, L, sizeof(L));
    memcpy(&DB[223 - m_len], m, sizeof(m));
    DB[222 - m_len] = 0x01;
    ...
    memcpy(MaskedDB, DB, 223);
    for (uint32_t i = 0; i < 223; i++) {
        MaskedDB[i] ^= DBMask[i];
    }
    ...
    uint64_t operand[32];
    memcpy(operand, EM, 256);

    endian_from_little_to_big(operand);
    endian_from_little_to_big(n);
    endian_from_little_to_big(e);
    ...
    uint64_t aR[32] = {0};
    montgomery_multiplication_v2(n, operand, R2, aR, neg_n_inverse);
    ...
    fwrite(ans_pow, sizeof(uint64_t), 32, out);
    return 0;
}